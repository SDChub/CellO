...
def load(features, rsrc_loc):
    ...
    return (
        og, # 本体数据
        label_graph, # 细胞label_id 层级图
        label_to_name, # 细胞label_id 到 细胞类型名称的映射
        the_exps, # 实验样本ID
        exp_to_index, # 实验样本ID 到 索引的映射
        exp_to_labels, # 实验样本ID 到 细胞类型ID的映射：一个实验样本, 对应多个细胞 label_id. 因为一个具体细胞类型可能属于很多细胞类型, 一个细胞类型可能包含多个具体细胞类型
        exp_to_tags, # 技术标签：说明该 实验样本ID 使用的技术("poly_a_rna", "uncultured")
        exp_to_study, # 实验样本ID 到 研究ID的映射
        study_to_exps, # 研究ID 到 实验样本ID的映射
        exp_to_ms_labels, # 实验样本ID 到 最具体细胞类型的映射. 应该就是最终使用细胞label
        data_matrix, # 表达数据矩阵
        gene_ids # 基因ID
    )
