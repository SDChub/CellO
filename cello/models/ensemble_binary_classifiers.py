...
class EnsembleOfBinaryClassifiers(object):
    ...
    def fit(
        ...
        # 处理每个标签,  会给每个标签分别训练一个分类器
        self.trivial_labels = set()
        for label_i, label in enumerate(label_to_items.keys()):
            # 计算训练集
            pos_items, neg_items = self._compute_training_set(
                # label : 每条item对应的label
                # train_items : experiment(cell) 索引 
                # label_to_items : label{i} : item1 item2  item3
                # item_to_labels : item{i} : label1 label2 label3
                # label_graph : 细胞类型的层级关系图
                label, train_items, label_to_items, item_to_labels, label_graph
            )
            ...
            if len(pos_items) > 0 and len(neg_items) == 0:
                print(
                    "Skipped training classifier for label {}. No negative examples.".format(
                        label
                    )
                )
    ...
    # 
    def _compute_training_set(self, label, train_items, label_to_items, item_to_labels, label_graph):

        # label : 每条item对应的 label
        # train_items :  experiment(cell) 索引 
        # label_to_items : label{i} : item
        # item_to_labels : item : label1 label2 label3
        # label_graph : 细胞类型的层级关系图

        # 计算正样本
        pos_items = _compute_positive_examples(label, label_to_items, label_graph)
        # 计算负样本
        neg_items = _compute_negative_examples(label,train_items,pos_items,item_to_labels,label_graph,self.assert_ambig_neg,  # 默认为False
        )
    ...
    def predict(self, X, test_items, verbose=True):
        ...
        for label in all_labels:
            ...
            for index, clss in enumerate(classifier.classes_):
                if clss == POS_CLASS:
                    pos_index = index
                    break
            scores = [x[pos_index] for x in classifier.predict_proba(X)]
            mat.append(scores)
        ...
        mat = np.array(mat).T
        df = pd.DataFrame(data=mat, index=test_items, columns=all_labels)
        return df, df
    ...
    def label_to_coefficients(self):
        return {
            label: classif.coef_ for label, classif in self.label_to_classifier.items()
        }
...
def _train_classifier(
    ...
    # 构建分类器
    model = bc.build_binary_classifier(binary_classif_algo, binary_classif_params)
    ...
    if group_weighted:
        ...
        item_to_group = {
            item: group for item, group in item_to_group.items() if item in all_items
        }
        # 计算权重
        group_to_size = Counter(item_to_group.values())
        sample_weights = [
            1.0 / group_to_size[item_to_group[item]] for item in pos_items + neg_items
        ]
...
# 计算正样本的辅助函数
def _compute_positive_examples(label, label_to_items, label_graph):
    """
    计算给定标签的正样本。
...
# 计算负样本的辅助函数
def _compute_negative_examples(label, all_items, pos_items, item_to_labels, label_graph, assert_ambig_neg):
    """
    计算给定标签的负样本集。
    这个集合包含所有未被标记为该标签后代，
    ...
    if assert_ambig_neg:
        ...
        for item in candidate_items:
            ms_item_labels = label_graph.most_specific_nodes(item_to_labels[item])
