...
def cello(
    ...
    cello.scanpy_cello(
        ...
        clust_key='leiden', # 聚类结果的键名
        rsrc_loc=cello_resource_loc,    # CellO资源文件位置
        out_prefix=model_prefix,        # 输出文件前缀
        log_dir=os.getcwd()             # 日志目录
    )
    """
    ...
    try:
        ...
        raise ImportError("需要安装cello包：请在终端运行 `pip install --user cello`")
    ...
    if model_file:
        ...
        # 如果调用者没有提供模型, 那么先尝试在目录下检索模型, 看看是否有符合要求的可用的模型
        # mod = ce._retrieve_pretrained_model(adata, algo, rsrc_loc)
        mod = None
        if mod is None:  # 如果实在没有可用的模型, 那么就重新训练一个模型
            # 如果没有预训练模型，则训练新模型
            mod = ce.train_model(adata, rsrc_loc, algo=algo, log_dir=log_dir)
            # 保存训练好的模型
            if out_prefix:
    ...
    if term_ids:
        # 使用术语ID作为列名
        column_to_term_id = {
            "{} (probability)".format(c): c for c in results_df.columns
        }
        results_df.columns = ["{} (probability)".format(c) for c in results_df.columns]
        finalized_binary_results_df.columns = [
            "{} (binary)".format(c) for c in finalized_binary_results_df.columns
        ]
    else:
        # 使用术语名称作为列名
        column_to_term_id = {
        ...
        results_df.columns = [
            "{} (probability)".format(ou.cell_ontology().id_to_term[c].name)
            for c in results_df.columns
        ]
        finalized_binary_results_df.columns = [
            "{} (binary)".format(ou.cell_ontology().id_to_term[c].name)
    ...
    # 将二值结果转换为分类变量
    finalized_binary_results_df = (
        finalized_binary_results_df.astype(bool).astype(str).astype("category")
    )
...
def normalize_and_cluster(
    ...
    try:
        ...
        sys.exit(
            "'normalize_and_cluster'函数需要安装scanpy包。请运行'pip install scanpy'进行安装"
        )
...
def cello_probs(
    ...
    if clust_key:
        ...
        results_df = pd.DataFrame(
            [results_df.loc[clust_to_indices[clust][0]] for clust in clusts],
            index=clusts,
            columns=results_df.columns,
        )

    # 生成概率图
    g = probabilities_on_graph(cell_or_clust, results_df, rsrc_loc, p_thresh=p_thresh)
